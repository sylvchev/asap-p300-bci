...
def calibration_mdm(ep, n_calibration, nfilter, est, xest):
...
def calibration_ppl(ep, n_calibration, sfreq, ppl):
    """Train any pipeline on n_calibration trials, return pipeline"""
    ep_calib, ev_tgt, ev_nontgt = [], [], []
    for i in range(n_calibration):
        ep_calib.append(ep[f"trial{i+1}"])
        ev_nontgt += [
...
def score_subject(ep, ppl, scorers, n_calibration, n_trials, sfreq=None):
    """Use ppl pipelines and scorers on ep epochs for n_trials"""
    results_char = []
    mdm_eval = {"TN": 0, "TP": 0, "FN": 0, "FP": 0}
    for trial in range(n_calibration, n_trials):
                    ...
                    results_char.append(
                        {
                            **res_header,
                            ...
                            "letter": char,
                            "scorer": ns,
                            "proba": sc.get_proba()[l2c_BNCI2014008(char)],
...
for subj in dataset.subject_list:
    ...
    baseline = -0.3, 0.0
    fmin, fmax = 0.1, 24
    ep = get_epochs(subj, tmin, tmax, baseline, fmin, fmax)
    ...
    n_calibration = 7
    resample_freq = 128
    ppl = calibration_mdm(ep, n_calibration, nfilter, est, xest)
    ...
    ppl_xd = calibration_ppl(ep, n_calibration, resample_freq, pxd)
    ...
    ppl_ld = calibration_ppl(ep, n_calibration, resample_freq, pld)
    ...
    scorers = {
        "pMDM+BA": Scorer_BA(symbols, c2l_BNCI2014008, l2c_BNCI2014008),
        "MDM+OM": Scorer_OM(symbols, c2l_BNCI2014008, l2c_BNCI2014008),
    }
    df_char = score_subject(ep, ppl, scorers, n_calibration, n_trials)
    df_allsubj.append(df_char)
    sc_xd = {
        "Xdawn+lda+OM": Scorer_OM(symbols, c2l_BNCI2014008, l2c_BNCI2014008),
    }
    df_xd = score_subject(ep, ppl_xd, sc_xd, n_calibration, n_trials, resample_freq)
    df_allsubj.append(df_xd)
    sc_lda = {
        "reglda+OM": Scorer_OM(symbols, c2l_BNCI2014008, l2c_BNCI2014008),
    }
    df_xd = score_subject(ep, ppl_ld, sc_lda, n_calibration, n_trials, resample_freq)
    df_allsubj.append(df_xd)

df_allsubj = pd.concat(df_allsubj)
df_allsubj.to_csv("bnci2014008.csv")
